// Voice Separator AudioWorkletProcessor
// Uses spectral subtraction and frequency filtering to isolate vocal frequencies

class VoiceSeparatorProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    
    // Parameters
    this.sensitivity = 0.5; // 0-1, how aggressive the separation is
    this.highPass = 80; // Hz, remove low frequencies below vocals
    this.lowPass = 8000; // Hz, remove high frequencies above vocals
    
    // FFT settings
    this.fftSize = 2048; // Larger FFT for better frequency resolution
    this.hopSize = this.fftSize / 4; // 75% overlap
    
    // Window function (Hann window)
    this.window = new Float32Array(this.fftSize);
    for (let i = 0; i < this.fftSize; i++) {
      this.window[i] = 0.5 - 0.5 * Math.cos(2 * Math.PI * i / (this.fftSize - 1));
    }
    
    // Per-channel buffers
    this.channelData = [];
    for (let ch = 0; ch < 2; ch++) { // Support stereo
      this.channelData[ch] = {
        inputBuffer: new Float32Array(this.fftSize),
        outputBuffer: new Float32Array(this.fftSize),
        overlapBuffer: new Float32Array(this.fftSize),
        real: new Float32Array(this.fftSize),
        imag: new Float32Array(this.fftSize),
        magnitudes: new Float32Array(this.fftSize / 2 + 1),
        phases: new Float32Array(this.fftSize / 2 + 1),
        smoothedMagnitudes: new Float32Array(this.fftSize / 2 + 1),
        inputPos: 0,
        outputPos: 0,
        hopCounter: 0
      };
    }
    
    // Vocal frequency emphasis curve (200Hz - 4kHz with peak around 1-2kHz)
    this.vocalCurve = new Float32Array(this.fftSize / 2 + 1);
    this.initializeVocalCurve();
  }
  
  initializeVocalCurve() {
    const sampleRate = 44100; // Assuming 44.1kHz, will be updated if different
    const binFrequency = sampleRate / this.fftSize;
    
    for (let i = 0; i <= this.fftSize / 2; i++) {
      const freq = i * binFrequency;
      
      // Create a curve that emphasizes vocal frequencies
      let curve = 0;
      
      // Main vocal range (200Hz - 4kHz)
      if (freq >= 200 && freq <= 4000) {
        // Peak around 1-2kHz (fundamental frequencies of vocals)
        if (freq >= 800 && freq <= 2500) {
          curve = 1.0;
        } else if (freq >= 200 && freq < 800) {
          // Gradual rise from 200Hz to 800Hz
          curve = (freq - 200) / 600;
        } else if (freq > 2500 && freq <= 4000) {
          // Gradual fall from 2500Hz to 4kHz
          curve = 1.0 - (freq - 2500) / 1500;
        }
      }
      
      // Presence frequencies (4kHz - 8kHz) - important for vocal clarity
      if (freq > 4000 && freq <= 8000) {
        curve = 0.3 * (1.0 - (freq - 4000) / 4000);
      }
      
      this.vocalCurve[i] = curve;
    }
  }
  
  // FFT implementation (same as spectral gate)
  fft(real, imag, inverse = false) {
    const n = real.length;
    const bits = Math.log2(n);
    
    // Bit reversal
    for (let i = 0; i < n; i++) {
      let j = 0;
      let x = i;
      for (let k = 0; k < bits; k++) {
        j = (j << 1) | (x & 1);
        x >>= 1;
      }
      if (j > i) {
        [real[i], real[j]] = [real[j], real[i]];
        [imag[i], imag[j]] = [imag[j], imag[i]];
      }
    }
    
    // Cooley-Tukey FFT
    for (let size = 2; size <= n; size *= 2) {
      const halfSize = size / 2;
      const angle = (inverse ? 2 : -2) * Math.PI / size;
      const wReal = Math.cos(angle);
      const wImag = Math.sin(angle);
      
      for (let start = 0; start < n; start += size) {
        let tmpReal = 1;
        let tmpImag = 0;
        
        for (let i = 0; i < halfSize; i++) {
          const even = start + i;
          const odd = even + halfSize;
          
          const evenReal = real[even];
          const evenImag = imag[even];
          const oddReal = real[odd];
          const oddImag = imag[odd];
          
          real[even] = evenReal + tmpReal * oddReal - tmpImag * oddImag;
          imag[even] = evenImag + tmpReal * oddImag + tmpImag * oddReal;
          real[odd] = evenReal - (tmpReal * oddReal - tmpImag * oddImag);
          imag[odd] = evenImag - (tmpReal * oddImag + tmpImag * oddReal);
          
          const nextTmpReal = tmpReal * wReal - tmpImag * wImag;
          const nextTmpImag = tmpReal * wImag + tmpImag * wReal;
          tmpReal = nextTmpReal;
          tmpImag = nextTmpImag;
        }
      }
    }
    
    // Normalize for inverse transform
    if (inverse) {
      const scale = 1 / n;
      for (let i = 0; i < n; i++) {
        real[i] *= scale;
        imag[i] *= scale;
      }
    }
  }
  
  // Process one frame for a channel
  processFrame(channel, sampleRate) {
    const data = this.channelData[channel];
    
    // Apply window to input
    for (let i = 0; i < this.fftSize; i++) {
      data.real[i] = data.inputBuffer[i] * this.window[i];
      data.imag[i] = 0;
    }
    
    // Forward FFT
    this.fft(data.real, data.imag, false);
    
    // Convert to magnitude and phase
    for (let i = 0; i <= this.fftSize / 2; i++) {
      data.magnitudes[i] = Math.sqrt(data.real[i] * data.real[i] + data.imag[i] * data.imag[i]);
      data.phases[i] = Math.atan2(data.imag[i], data.real[i]);
    }
    
    // Apply voice separation
    const binFrequency = sampleRate / this.fftSize;
    const highPassBin = Math.floor(this.highPass / binFrequency);
    const lowPassBin = Math.ceil(this.lowPass / binFrequency);
    
    for (let i = 0; i <= this.fftSize / 2; i++) {
      let gain = 0;
      
      // Apply frequency band filtering
      if (i >= highPassBin && i <= lowPassBin) {
        // Apply vocal curve with sensitivity
        gain = this.vocalCurve[i] * this.sensitivity;
        
        // Enhance bins with strong harmonic content (characteristic of vocals)
        if (i > 10 && i < this.fftSize / 4) {
          // Check for harmonic patterns
          const fundamental = i;
          let harmonicStrength = 0;
          let harmonicCount = 0;
          
          // Check first 4 harmonics
          for (let h = 2; h <= 5; h++) {
            const harmonicBin = fundamental * h;
            if (harmonicBin <= this.fftSize / 2) {
              harmonicStrength += data.magnitudes[harmonicBin];
              harmonicCount++;
            }
          }
          
          if (harmonicCount > 0) {
            harmonicStrength /= harmonicCount;
            // If harmonics are strong relative to fundamental, boost gain
            if (harmonicStrength > data.magnitudes[i] * 0.3) {
              gain = Math.min(1.0, gain * 1.5);
            }
          }
        }
      }
      
      // Apply gain to magnitude
      const newMagnitude = data.magnitudes[i] * gain;
      
      // Convert back to real/imag
      data.real[i] = newMagnitude * Math.cos(data.phases[i]);
      data.imag[i] = newMagnitude * Math.sin(data.phases[i]);
      
      // Mirror for negative frequencies (except DC and Nyquist)
      if (i > 0 && i < this.fftSize / 2) {
        data.real[this.fftSize - i] = data.real[i];
        data.imag[this.fftSize - i] = -data.imag[i];
      }
    }
    
    // Inverse FFT
    this.fft(data.real, data.imag, true);
    
    // Apply window to output and add to overlap buffer
    for (let i = 0; i < this.fftSize; i++) {
      data.outputBuffer[i] = data.real[i] * this.window[i];
      data.overlapBuffer[i] += data.outputBuffer[i];
    }
  }
  
  process(inputs, outputs, parameters) {
    const input = inputs[0];
    const output = outputs[0];
    
    if (!input || input.length === 0 || !output || output.length === 0) {
      return true;
    }
    
    // Update parameters
    if (parameters.sensitivity && parameters.sensitivity.length > 0) {
      this.sensitivity = parameters.sensitivity[0];
    }
    if (parameters.highPass && parameters.highPass.length > 0) {
      this.highPass = parameters.highPass[0];
    }
    if (parameters.lowPass && parameters.lowPass.length > 0) {
      this.lowPass = parameters.lowPass[0];
    }
    
    const numChannels = Math.min(input.length, output.length);
    const blockSize = input[0].length;
    
    // Process each channel
    for (let ch = 0; ch < numChannels; ch++) {
      const inputChannel = input[ch];
      const outputChannel = output[ch];
      const data = this.channelData[ch];
      
      if (!data) continue; // Skip if we don't have buffers for this channel
      
      // Process each sample
      for (let i = 0; i < blockSize; i++) {
        // Add input sample to buffer
        data.inputBuffer[data.inputPos] = inputChannel[i];
        data.inputPos++;
        
        // Check if we have enough samples to process a frame
        if (data.inputPos >= this.hopSize && data.hopCounter === 0) {
          // Process frame
          this.processFrame(ch, sampleRate);
          
          // Shift input buffer by hop size
          for (let j = 0; j < this.fftSize - this.hopSize; j++) {
            data.inputBuffer[j] = data.inputBuffer[j + this.hopSize];
          }
          data.inputPos = this.fftSize - this.hopSize;
          
          // Reset hop counter
          data.hopCounter = this.hopSize;
        }
        
        // Output from overlap buffer
        if (data.hopCounter > 0) {
          outputChannel[i] = data.overlapBuffer[data.outputPos];
          data.outputPos++;
          data.hopCounter--;
          
          // If we've output a hop's worth of samples, shift the overlap buffer
          if (data.hopCounter === 0) {
            // Shift overlap buffer
            for (let j = 0; j < this.fftSize - this.hopSize; j++) {
              data.overlapBuffer[j] = data.overlapBuffer[j + this.hopSize];
            }
            // Clear the end
            for (let j = this.fftSize - this.hopSize; j < this.fftSize; j++) {
              data.overlapBuffer[j] = 0;
            }
            data.outputPos = 0;
          }
        } else {
          // Output silence until we have processed data
          outputChannel[i] = 0;
        }
      }
    }
    
    return true;
  }
  
  static get parameterDescriptors() {
    return [{
      name: 'sensitivity',
      defaultValue: 0.5,
      minValue: 0,
      maxValue: 1,
      automationRate: 'k-rate'
    }, {
      name: 'highPass',
      defaultValue: 80,
      minValue: 20,
      maxValue: 1000,
      automationRate: 'k-rate'
    }, {
      name: 'lowPass',
      defaultValue: 8000,
      minValue: 1000,
      maxValue: 20000,
      automationRate: 'k-rate'
    }];
  }
}

registerProcessor('voice-separator-processor', VoiceSeparatorProcessor);